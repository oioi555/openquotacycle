...
impl<T: Tray> StatusNotifierItem<T> {
    ...
    /// Plasma sends this immediately before Activate / dbusmenu clicks so the
    /// window can consume an xdg-activation token on Wayland.
    async fn provide_xdg_activation_token(
        ...
        let mut service = self.0.lock().await; // do NOT use any self methods after this
        service
            .call_provide_xdg_activation_token(conn, token)
            .await;
        Ok(())
    }
