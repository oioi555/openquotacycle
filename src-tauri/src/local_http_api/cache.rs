...
pub fn load_cache(app_data_dir: &Path) -> HashMap<String, CachedPluginSnapshot> {
    ...
    match serde_json::from_str::<UsageApiCacheFile>(&data) {
        // Older Go snapshots contain unmarked dollar estimates. Require a fresh
        // probe after startup rather than presenting those as authoritative quota.
        Ok(file) if file.version == 1 => file
            .snapshots
            .into_iter()
            .filter(|(provider_id, snapshot)| {
                provider_id != OPENCODE_GO_PROVIDER_ID
                    && snapshot.provider_id != OPENCODE_GO_PROVIDER_ID
            })
            .collect(),
        Ok(_) => {
...
mod tests {
    ...
    fn load_cache_skips_persisted_opencode_go_snapshot() {
        ...
        snapshots.insert(
            ...
            make_snapshot(OPENCODE_GO_PROVIDER_ID, "OpenCode Go"),
        );
        snapshots.insert("claude".to_string(), make_snapshot("claude", "Claude"));
        ...
        assert!(!loaded.contains_key(OPENCODE_GO_PROVIDER_ID));
        assert_eq!(loaded.len(), 1);
        assert_eq!(loaded["claude"].provider_id, "claude");
