...
fn configure_child_process_group(command: &mut Command) {
    ...
    // Package runners spawn the actual ccusage process. Put the whole runner tree
    // in its own group so timeout cleanup also closes inherited stdout/stderr pipes.
    unsafe {
        command.pre_exec(|| {
...
fn terminate_ccusage_process(child: &mut Child) {
    #[cfg(unix)]
    {
        let process_group = -(child.id() as libc::pid_t);
        // Ignore ESRCH: the group may have exited before timeout cleanup ran.
        unsafe {
            libc::kill(process_group, libc::SIGKILL);
        }
    }

    // Keep the direct-child fallback for platforms without process groups and
    // for runners that exit between the group and child termination calls.
    let _ = child.kill();
}
        ...
        match child.try_wait() {
            Ok(Some(status)) => {
                ...
                if status.success() {
                    ...
                    if let Some(normalized_json) = normalize_ccusage_output(&out) {
                        return CcusageAttempt::Success(normalized_json);
                    }
                    log::warn!(
            ...
            Ok(None) => {
                if start.elapsed() > timeout {
                    terminate_ccusage_process(&mut child);
                    let _ = child.wait();
                    ...
                    return CcusageAttempt::TimedOut;
                }
        ...
        Function::new(
            ...
            move |_ctx_inner: Ctx<'_>, opts_json: String| -> rquickjs::Result<String> {
                ...
                for (kind, program) in runners {
                    match run_ccusage_with_runner(kind, &program, &opts, provider, &pid) {
                        CcusageAttempt::TimedOut => {
                            // Other package runners are subject to the same network/install
                            // delay; do not multiply a timeout across every runner.
                            return Ok(serde_json::json!({ "status": "timeout" }).to_string());
                        }
                        CcusageAttempt::Failed => continue,
                        CcusageAttempt::Success(result) => {
...
pub fn patch_ccusage_wrapper(ctx: &rquickjs::Ctx<'_>) -> rquickjs::Result<()> {
    ctx.eval::<(), _>(
        r#"
        (function() {
            var rawFn = __quotracker_ctx.host.ccusage._queryRaw;
            __quotracker_ctx.host.ccusage.query = function(opts) {
...
mod tests {
    ...
    fn ccusage_path_entries_with_home_and_existing_path_preserves_order() {
        let home = std::path::PathBuf::from("/tmp/quotracker-home");
    ...
    #[cfg(unix)]
    #[test]
    fn ccusage_process_group_terminates_runner_children() {
        use std::io::BufRead;
        use std::sync::mpsc;
        use std::thread;
        ...
        let mut child = command.spawn().expect("spawn process group test");
        ...
        let descendant_pid: libc::pid_t =
            descendant_pid.trim().parse().expect("parse descendant pid");
        ...
        terminate_ccusage_process(&mut child);
        let _ = child.wait();
        let pipes_closed = done_rx.recv_timeout(Duration::from_secs(2)).is_ok();

        if !pipes_closed {
            // Keep the test bounded even if process-group setup regresses.
            unsafe {
        ...
        assert!(
            pipes_closed,
            "runner descendants must not retain ccusage pipes"
        );
    }
