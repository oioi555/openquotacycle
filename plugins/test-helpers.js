...
export const makeCtx = () => {
    ...
    host: {
      ...
      crypto: {
        decryptAes256Gcm: vi.fn(() => {
          throw new Error("decryptAes256Gcm mock not configured")
        }),
        encryptAes256Gcm: vi.fn(() => {
          throw new Error("encryptAes256Gcm mock not configured")
        }),
        sha256Hex: vi.fn((text) => sha256HexSync(String(text))),
      },
      sqlite: {
        query: vi.fn(() => "[]"),
        exec: vi.fn(),
  ...
  ctx.status = {
    chip: (opts) => ({ text: opts.text, tone: opts.tone }),
  }
