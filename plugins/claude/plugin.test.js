...
describe("claude plugin", () => {
  it("declares Window Starter on Claude Code only", () => {
  ...
  it("prefers stored file OAuth over CLAUDE_CODE_OAUTH_TOKEN for live meters", async () => {
    const ctx = makeCtx()
    ctx.host.fs.exists = () => true
    ctx.host.fs.readText = () =>
      JSON.stringify({ claudeAiOauth: { accessToken: "file-oauth-token", subscriptionType: "pro" } })
    ctx.host.env.get.mockImplementation((name) =>
      name === "CLAUDE_CODE_OAUTH_TOKEN" ? "env-oauth-token" : null
    )
    ctx.host.http.request.mockReturnValue({
    ...
    expect(usageCall[0].headers.Authorization).toBe("Bearer file-oauth-token")
  })

  it("emits Fable from weekly_scoped limits", async () => {
    const ctx = makeCtx()
    ctx.host.fs.exists = () => true
    ctx.host.fs.readText = () =>
      JSON.stringify({ claudeAiOauth: { accessToken: "token", subscriptionType: "pro" } })
    ...
    expect(fable.used).toBe(18)
    expect(fable.periodDurationMs).toBe(7 * 24 * 60 * 60 * 1000)
  })
  ...
  it("renders usage lines from response", async () => {
    ...
    expect(
      ctx.host.http.request.mock.calls.some(
        (call) => String(call[0]?.url) === "https://promoclock.co/api/status"
      )
    ).toBe(false)
  })
  ...
  it("returns empty lines when no usage data and ccusage is unavailable", async () => {
    const ctx = makeCtx()
    ctx.host.fs.readText = () => JSON.stringify({ claudeAiOauth: { accessToken: "token" } })
    ctx.host.fs.exists = () => true
    ctx.host.http.request.mockReturnValue({
