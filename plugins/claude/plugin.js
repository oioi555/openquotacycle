(function () {
  ...
  function loadCredentials(ctx) {
    ...
    if (stored && stored.oauth && stored.oauth.accessToken) {
      return stored
    }
    if (!envAccessToken) return stored
    return {
    ...
    if (data) {
      ...
      if (Array.isArray(data.limits)) {
        for (let i = 0; i < data.limits.length; i++) {
          ...
          if (!entry || entry.kind !== "weekly_scoped" || !model || model.display_name !== "Fable") continue
