(function () {
  ...
  function additionalLimitHay(entry) {
    ...
    return (name + " " + feature).toLowerCase()
  }

  function isSparkLimit(entry) {
    const hay = additionalLimitHay(entry)
    return hay.includes("codex-spark") || hay.includes("bengalfox")
  }

  function isLunaReserveLimit(entry) {
    const hay = additionalLimitHay(entry)
    return hay.includes("gpt-reserve") || hay.includes("luna-reserve")
  }

  // Luna Reserve is the official name for the gpt-reserve fallback pool.
  // Keep it on a stable label so plugin.json can classify it On Demand.
  function additionalLimitLabel(entry) {
    if (isLunaReserveLimit(entry)) return "Luna Reserve"
    const name = typeof entry.limit_name === "string" ? entry.limit_name : ""
  ...
  function probe(ctx) {
    ...
    if (auth.tokens && auth.tokens.access_token) {
      ...
      // Account-level rate_limit windows are classified by duration for every
      // plan: 18000 → Session, 604800 → Weekly. Unknown durations are omitted.
      const accountWindows = [
        { window: primaryWindow, header: headerPrimary },
        { window: secondaryWindow, header: headerSecondary },
      ]
      let sessionCandidate = null
      let weeklyCandidate = null
      for (const accountWindow of accountWindows) {
        const win = accountWindow.window
        if (!win) continue
        const limit = win.limit_window_seconds
        if (limit === 604800) {
          if (weeklyCandidate) continue
          const used = accountWindow.header !== null ? accountWindow.header : win.used_percent
          if (typeof used === "number") {
            weeklyCandidate = { win: win, used: used }
          }
        } else if (limit === 18000) {
          if (sessionCandidate) continue
          const used = accountWindow.header !== null ? accountWindow.header : win.used_percent
          if (typeof used === "number") {
            sessionCandidate = { win: win, used: used }
          }
        }
      }
      if (sessionCandidate) {
        lines.push(ctx.line.progress({
          label: "Session",
          used: sessionCandidate.used,
          limit: 100,
          format: { kind: "percent" },
          resetsAt: getResetsAtIso(ctx, nowSec, sessionCandidate.win),
          periodDurationMs: sessionCandidate.win.limit_window_seconds * 1000
        }))
      }
      if (weeklyCandidate) {
        lines.push(ctx.line.progress({
          label: "Weekly",
          used: weeklyCandidate.used,
          limit: 100,
          format: { kind: "percent" },
          resetsAt: getResetsAtIso(ctx, nowSec, weeklyCandidate.win),
          periodDurationMs: weeklyCandidate.win.limit_window_seconds * 1000
        }))
      }
          ...
          if (isLunaReserveLimit(entry)) {
            ...
            if (weekly) {
              lines.push(ctx.line.progress({
                ...
                limit: 100,
                format: { kind: "percent" },
                resetsAt: getResetsAtIso(ctx, nowSec, weekly),
                periodDurationMs: weekly.limit_window_seconds * 1000
              }))
            }
            continue
          }
          const shortName = additionalLimitLabel(entry)
      ...
      if (resetCredits && typeof resetCredits === "object") {
        const available = readNumber(resetCredits.available_count)
        if (available !== null && available >= 0) {
          lines.push(ctx.line.text({
            label: "Rate Limit Resets",
            value: String(Math.floor(available)) + " available",
          }))
        }
      }
      ...
      if (creditsRemaining !== null) {
        const credits = Math.max(0, Math.floor(creditsRemaining))
        const usd = credits * 0.04
        lines.push(ctx.line.text({
          label: "Extra Usage",
          value: "$" + usd.toFixed(2) + " · " + String(credits) + " credits",
        }))
      }
