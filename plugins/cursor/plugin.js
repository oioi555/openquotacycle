(function () {
  ...
  const GROK_BOT_URL = BASE_URL + "/aiserver.v1.DashboardService/GetSandUsageStatus"
  const REST_USAGE_URL = "https://cursor.com/api/usage"
  const STRIPE_URL = "https://cursor.com/api/auth/stripe"
  const CLIENT_ID = "KbZUR41cY7W6zRSdpSUJ7I7mLYBKOCmB"
  const REFRESH_BUFFER_MS = 5 * 60 * 1000 // refresh 5 minutes before expiration
  const LOGIN_HINT = "Sign in via the Cursor app."

  function stateDbPath(ctx) {
    return ctx.app.platform === "linux" ? LINUX_STATE_DB : MACOS_STATE_DB
  }
  ...
  function probe(ctx) {
    ...
    const hasTotalPercent = hasPlanUsage &&
    ...
    const hasPoolUsage = hasPlanUsage && (
      (typeof usage.planUsage.autoPercentUsed === "number" && Number.isFinite(usage.planUsage.autoPercentUsed)) ||
      (typeof usage.planUsage.apiPercentUsed === "number" && Number.isFinite(usage.planUsage.apiPercentUsed))
    )
    const planUsageUnusable = !hasPlanUsage || (planUsageLimitMissing && !hasTotalPercent && !hasPoolUsage)
    ...
    const needsRequestBasedFallback = usage.enabled !== false && planUsageUnusable && (
    ...
    if (usage.enabled !== false && planUsageUnusable) {
    ...
    if (typeof pu.totalPercentUsed === "number" && Number.isFinite(pu.totalPercentUsed)) {
      lines.push(ctx.line.progress({
        label: "Total Usage",
    ...
    try {
      const grokResp = connectPost(ctx, GROK_BOT_URL, accessToken)
      if (grokResp.status >= 200 && grokResp.status < 300) {
        const grok = ctx.util.tryParseJson(grokResp.bodyText)
        if (
          grok &&
          grok.usesPooledEnterpriseAllowance !== true &&
          grok.hasNonZeroIncludedLimit !== false &&
          grok.includedLimitZero !== true &&
          typeof grok.usagePercent === "number" &&
          Number.isFinite(grok.usagePercent) &&
          grok.usagePercent >= 0
        ) {
          const grokOpts = {
            label: "Grok Bot",
            used: Math.min(100, grok.usagePercent),
            limit: 100,
            format: { kind: "percent" },
            periodDurationMs: 7 * 24 * 60 * 60 * 1000,
          }
          const grokReset = ctx.util.toIso(grok.nextResetTimestampUtc)
          if (grokReset) grokOpts.resetsAt = grokReset
          lines.push(ctx.line.progress(grokOpts))
        }
      } else {
        ctx.host.log.warn("optional Grok Bot usage request returned HTTP " + grokResp.status)
      }
    } catch (e) {
      ctx.host.log.warn("optional Grok Bot usage request failed: " + String(e))
    }
