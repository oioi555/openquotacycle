...
Quotracker can route provider and plugin HTTP requests through an optional proxy.
...
Create `~/.config/quotracker/config.json`:
...
- Restart Quotracker after changing the file.
...
This applies to provider and plugin HTTP requests that go through Quotracker's built-in HTTP client.
