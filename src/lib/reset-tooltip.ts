...
function formatMonthDayNumeric(timestampMs: number): string {
  const date = new Date(timestampMs)
...
export function formatResetRelativeFace(nowMs: number, resetsAtIso: string): string | null {
  const resetsAtMs = parseResetTimestamp(resetsAtIso)
  if (resetsAtMs === null) return null
  const deltaMs = resetsAtMs - nowMs
  if (deltaMs < RESET_SOON_THRESHOLD_MS) return "soon"
  return formatCompactDuration(deltaMs)
}
...
export function formatResetAbsoluteFace(nowMs: number, resetsAtIso: string): string | null {
  const resetsAtMs = parseResetTimestamp(resetsAtIso)
  if (resetsAtMs === null) return null
  if (resetsAtMs - nowMs <= 0) return "soon"
  const timeText = RESET_TIME_FORMATTER.format(resetsAtMs)
  const dayDiff = getLocalDayIndex(resetsAtMs) - getLocalDayIndex(nowMs)
  if (dayDiff <= 0) return timeText
  return `${formatMonthDayNumeric(resetsAtMs)} ${timeText}`
}
