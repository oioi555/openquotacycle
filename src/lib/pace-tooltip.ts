...
/** Compact ETA for the run-out chip face, e.g. "4d 5h". */
export function formatRunsOutFace({
  paceResult,
  used,
  limit,
  periodDurationMs,
  ...
  return getRunsOutDurationText({ paceResult, used, limit, periodDurationMs, resetsAtMs, nowMs })
}
...
export function formatRunsOutText({
  ...
  const durationText = formatRunsOutFace({ paceResult, used, limit, periodDurationMs, resetsAtMs, nowMs })
