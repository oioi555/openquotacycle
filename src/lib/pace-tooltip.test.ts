...
describe("formatRunsOutText", () => {
  ...
  it("formats compact run-out face without a prefix", () => {
    const paceResult: PaceResult = { status: "behind", projectedUsage: 120 }
    const args = {
      paceResult,
      used: 60,
    ...
    expect(formatRunsOutFace(args)).toBe("8h 0m")
    expect(formatRunsOutFace(args)).not.toMatch(/Runs out/)
    expect(formatRunsOutText(args)).toBe("Runs out in 8h 0m")
  })
