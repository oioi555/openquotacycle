...
- `choices.md` / `breadcrumbs.md` are local-only (gitignored): write entries in Japanese; keep code identifiers, paths, and commands verbatim.
...
- Never use the word `dump` in UI, README, or live specs. Crossing-go copy is `N% ahead of pace · melts at reset`. Desktop notify title is `Leftover melting`.
