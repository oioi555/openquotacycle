...
The Customize screen SHALL let users reorder a provider's overview progress lines by dragging them, and the persisted order SHALL determine the row order of those lines on the dashboard card, both collapsed and expanded. Labels without a stored order position SHALL keep their manifest order after the stored ones. The ordering SHALL persist across restarts.

#### Scenario: Dragging reorders dashboard rows

- **WHEN** the user drags a progress line to a new position in the provider's Customize detail view
- **THEN** the dashboard card renders that provider's progress lines in the new order

#### Scenario: Order persists across restarts

- **WHEN** the user reorders a provider's progress lines and restarts the application
- **THEN** the dashboard card keeps the reordered layout

#### Scenario: New manifest lines append to the stored order

- **WHEN** a provider's manifest gains a progress line that has no stored order position
...
Each dashboard provider card SHALL provide an expand control. When expanded, the card SHALL reveal the provider's On-Demand metric lines (those classified as not always visible) and its quick links; when collapsed, those lines and links SHALL be omitted. Cards SHALL be collapsed by default, and expansion state SHALL persist while the application runs.
...
the card's expand caret. No progress line is locked visible — any progress
line, including the first, may be hidden. Badge lines remain visible when
...
- **WHEN** a provider exposes a metric line outside the `overview` scope and
  ...
  manifest default (badge lines remain visible)
...
- **THEN** the Overview page omits that text line while continuing to
  show selected progress bars, other text lines, and badge lines
...
visibility controls. Each progress line SHALL be independently classifiable
as Always Visible or On-Demand — no progress line SHALL be locked to Always
Visible. Each text line SHALL also be independently classifiable — there is
no single per-provider statistics classification. A line's default
...
- **THEN** the provider's Customize detail view shows each progress line as
  an independently classifiable item
...
- **THEN** the provider's Customize detail view shows each text line as an
  independently classifiable item, and hiding one hides only that line from
  the collapsed dashboard card
...
- **THEN** the Customize detail view shows no progress-bar choices and the
