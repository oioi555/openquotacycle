...
- **THEN** the plugin makes no Google OAuth refresh request and returns a non-empty actionable error directing the user to start Antigravity or `agy`
...
- **WHEN** no local or Cloud Code response contains usable supported model quota data and no unexpired access token is available
