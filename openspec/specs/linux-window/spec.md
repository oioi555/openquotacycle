...
- **THEN** OpenQuotaCycle toggles the main window: hidden, minimized, or unfocused windows are shown, raised, and focused; a visible focused window is hidden
...
- **THEN** OpenQuotaCycle shows the main window, raises it above other windows, and focuses it
...
- **THEN** OpenQuotaCycle raises the main window above other windows and focuses it
...
- **THEN** OpenQuotaCycle hides the main window and keeps running with a visible status icon

#### Scenario: Panel-only behavior removed

- **WHEN** the main window loses focus or Escape is pressed
- **THEN** OpenQuotaCycle does not auto-hide the window through panel-specific logic

#### Scenario: Status icon is a StatusNotifierItem

- **WHEN** OpenQuotaCycle is running on a desktop that hosts StatusNotifierWatcher
- **THEN** a StatusNotifierItem for OpenQuotaCycle is registered
...
OpenQuotaCycle SHALL keep running when the user closes the main window: the close request MUST hide the window instead of terminating the application, so the tray icon, tray menu, and background usage refresh remain active.
...
OpenQuotaCycle SHALL toggle main window visibility with the global shortcut: hidden, minimized, or unfocused windows are shown, raised, and focused; a visible focused window is hidden.
